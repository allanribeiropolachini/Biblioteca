...
public class Aluno implements Pessoa {
    ...
    public Aluno(String n, int d, int m, int an, int cadDia, int cadMes, int cadAno) {
        nome = n;
        dia = d;
        mes = m;
        ano = an;
    ...
    public Livro devolverLivro(String nome) {
        ...
        if (indexLivro == -1) {
            System.out.println("Esse usuario nao existe ou nao tem esse livro!");
        } else {
    ...
    public boolean podeRetirar() {
        return !((livros.size() >= 4) || (bloqueado));
    }
