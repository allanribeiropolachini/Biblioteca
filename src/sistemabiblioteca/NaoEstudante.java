...
public class NaoEstudante implements Pessoa {
    ...
    public NaoEstudante(String n, int d, int m, int an, int cadDia, int cadMes, int cadAno) {
        nome = n;
        dia = d;
        mes = m;
        ano = an;
    ...
    public boolean retirarLivro(Livro l, int vencDia, int vencMes, int vencAno, int empDia, int empMes, int empAno) {
        if ("TEXTO".equals(l.getTipo())) {
            System.out.println("Este usuario nao pode retirar esse tipo de livro!");
            return false;
        } else {
            if (this.podeRetirar()) {
                livros.add(l);
    ...
    public boolean podeRetirar() {
        return !((livros.size() >= 2) || (bloqueado));
    }
